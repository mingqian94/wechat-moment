...
def execute_publish(task: dict) -> dict:
    ...
    # 图片 20s；视频 60s——2026-07-02 实测视频被风控拒发时，"视频未发送"横幅出现在点发表
    # 几分钟后，20s 窗口检测不到，误报了成功。
    # ⚠ 已知局限：60s 只是检测成本和漏检风险的折中，横幅若在 60s 后才出现仍会误报 SUCCESS
    #   （视频比图片更容易踩这个坑）。彻底解决要"发布后回头巡检朋友圈"确认最新一条状态，
    #   目前没做——所以视频的"发布成功"存在误报可能，发视频后建议人工抽查一下朋友圈。
    # 同时扫主窗口，防止 toast 出现在主窗口而非朋友圈弹窗。
    check_timeout = 60.0 if media_type == "video" else 20.0
