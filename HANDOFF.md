...
|---|---|---|
| 原理 | 一台电脑多开微信客户端，图像识别+模拟点击操作 PC 窗口 | PC 通过 ADB 驱动**真机**安卓手机，模拟点击操作手机上的微信；iPhone 只做半自动唤起 |
| 风控隔离性 | 差——已实测确认"一台 PC 多开 2 个微信"会触发风控（照片/视频发不出去） | 好——每台手机独立设备指纹，可各走 SIM 流量，天然对应客户"20 部真机"的场景 |
| 成熟度 | 较成熟，能发图/视频，还有个视频误报的尾巴没根治 | Android 核心链路已验证（多图、单视频、中文emoji文案、从微信首页进朋友圈、发布后标记待确认）；iPhone 已验证可识别设备、复制文案、打开微信，但不能远程点选/发表 |
| 结论倾向 | 更适合"一台电脑管 1-2 个号"的轻量场景 | Android 全自动是重点推进方向；客户 iPhone 多时，短期只能交付“定时提醒+复制文案+打开微信”的半自动模式 |
...
7. **iPhone 不等于 Android 自动化能力**。2026-07-08 在 iPhone13,3 / iOS 26.5 上验证：
...
- 手机版：Python 3.12，`pip install -r phone/requirements.txt`。开发/打包需要 Google 官方
  ...
  有几个必须踩的坑）。
- iPhone 半自动模式需要 iTunes/Apple Mobile Device Support + `pymobiledevice3`。手机需开启
  开发者模式并信任这台电脑。当前只验证 USB 连接；换电脑需要重新信任/重新开启开发者相关能力。
...
- iPhone 全自动发布仍未解决：需要签名 WDA Runner 或等待/验证 iOS 27+ CoreDevice 远程触控；
  交付时只能按半自动说明使用。
